...
- **`hourlyFrom`/`hourlyTo` in ES:** RESOLVED — Separate ES fields required. The existing Jobs alert workers Config.php (lines 109-122) explicitly defines two separate range field objects: `salary` → `salaryFrom`/`salaryTo` and `hourly` → `hourlyFrom`/`hourlyTo`. Comment notes "Jobs range fields are unique in that unlike other verticals that have 1 field on their listing (i.e. 'price'), they have 2 fields that match." The unfinished `jobs.go` percolation code also treats them as distinct fields. Classifieds ES mapping should add all four fields: `salaryFrom`, `salaryTo`, `hourlyFrom`, `hourlyTo` (float), with `payRangeType` (keyword) as qualifier to determine which range applies per listing
