...
| Method | Path | Purpose |
|--------|------|---------|
| POST | `/application` | Submit application (multipart; scans + uploads files, persists, emails employer). **No applicant/sender confirmation email** — out of scope (DECIDED 2026-07-08). |
...
- **One** new Mailgun template needed: employer notification. (~~Applicant confirmation template~~ dropped — no applicant/sender confirmation email, DECIDED 2026-07-08.)
...
### Applicant Confirmation — ❌ OUT OF SCOPE (DECIDED 2026-07-08)
- **No confirmation email is sent to the applicant/sender.** The applicant's only confirmation is the on-screen **"Thank you for applying" modal** (Feature 7, screen 5). Matches the Notion callout: *"No email confirmations — just a modal when the user submits applications."*
- Legacy behavior (not reimplemented): sent a confirmation email "Your application for [Job Title] is on its way!" with job title, company name, location, posted date.
...
- **2026-07-08 (applicant/sender confirmation email removed)**: User decided the applicant/sender confirmation email is **out of scope** — no email is sent to the applicant; they get the on-screen **"Thank you for applying" confirmation modal** only. Confirmed against Notion, which already reflects this (framing scope, feature list, acceptance criteria, and Feature 5 all struck/updated with a *"🚫 No email confirmations — just a modal"* callout). Updated the shaped pack to match: **Features.md** (Feature 5 → out of scope; Feature 7 screen 5 note), **Services.md** (email dependency + dropped the `jobs-application-confirmation` Mailgun template, now one template), **Slices.md** (Slice 3 → removed but slot retained to avoid renumbering; dep graph, estimates table, and subtotal/overall adjusted to ~4.0–6.2 wk), and this doc. Also reconciled the remaining Notion technical sections that still listed it (endpoint description, Mailgun templates, Slices dependency order + Slice 3 detail, estimates table).
